...
void fast_atomic_dw_load(volatile uint64_t atomic[2], uint64_t target[2])
{
	ConcurrentFW::atomic_dw_load(atomic, target);
}

void fast_atomic_dw_store(volatile uint64_t atomic[2], const uint64_t desired[2])
{
...
void fast_atomic_dw_load(volatile uint32_t atomic[2], uint32_t target[2])
{
	ConcurrentFW::atomic_dw_load(atomic, target);
}

void fast_atomic_dw_store(volatile uint32_t atomic[2], const uint32_t desired[2])
{
	ConcurrentFW::atomic_dw_store(atomic, desired);
}
