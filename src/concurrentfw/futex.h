...
protected:
	...
	static ALWAYS_INLINE int syscall_futex(volatile int* addr1, int op, int val1, const struct timespec* timeout, volatile int* addr2, int val3)
	{
		return syscall(SYS_futex, addr1, op, val1, timeout, addr2, val3);
	}

	static ALWAYS_INLINE int syscall_futex(volatile int* addr1, int op, int val1, uint32_t val2, volatile int* addr2, int val3)
	{
		return syscall(SYS_futex, addr1, op, val1, reinterpret_cast<void*>(static_cast<uintptr_t>(val2)), addr2, val3);
	}

	ALWAYS_INLINE int futex_wait(int expected, const struct timespec* timeout_relative = nullptr)
	{
		return syscall_futex(&value.atomic, FUTEX_WAIT_PRIVATE, expected, timeout_relative, nullptr, 0);
	}

	ALWAYS_INLINE int futex_wake(int wakeups = 1)
	{
	...
	ALWAYS_INLINE int futex_cmp_requeue(int wakeups, uint32_t limit, volatile int* target, int expected)
	{
	...
	{
		const uint32_t val3 =
			(static_cast<uint32_t>(oparg_shift) << 31) |
			(static_cast<uint32_t>(op) << 28) |
			(static_cast<uint32_t>(cmp) << 24) |
			(static_cast<uint32_t>(oparg & 0xfff) << 12) |
			(static_cast<uint32_t>(cmparg & 0xfff));
		return syscall_futex(&value.atomic, FUTEX_WAKE_OP_PRIVATE,
				wakeups1,
				wakeups2,
				address2,
	...
	ALWAYS_INLINE int futex_wait_bitset(uint32_t mask, int expected, struct timespec* timeout_absolute)
	{
		return syscall_futex(&value.atomic, FUTEX_WAIT_BITSET_PRIVATE, expected, timeout_absolute, nullptr, static_cast<int>(mask));
	}

	ALWAYS_INLINE int futex_wake_bitset(uint32_t mask, int wakeups = 1)
	{
		return syscall_futex(&value.atomic, FUTEX_WAKE_BITSET_PRIVATE, wakeups, nullptr, nullptr, static_cast<int>(mask));
	}
...
public:
	...
	ALWAYS_INLINE void lock(void)
	{
		...
		if(value.compare_exchange_strong<AtomicMemoryOrder::ACQUIRE,AtomicMemoryOrder::RELAXED>(expected_found, State::LOCKED_NOWAITERS) == false) [[unlikely]]	// 1: desired value: locked, no other waiting
	...
	ALWAYS_INLINE bool trylock(void)
	{
		int expected = State::UNLOCKED;			// 0: expected value: unlocked
		// memory order: critical section below, therefore to an atomic-aquire here here (but only in case of success)
		return value.compare_exchange_strong<AtomicMemoryOrder::ACQUIRE,AtomicMemoryOrder::RELAXED>(expected, State::LOCKED_NOWAITERS);	// 1: desired value: locked, no other waiting
	}
	...
	ALWAYS_INLINE bool trylock_timeout(const struct timespec* timeout_relative)
	{
		...
		if(value.compare_exchange_strong<AtomicMemoryOrder::ACQUIRE,AtomicMemoryOrder::RELAXED>(expected_found, State::LOCKED_NOWAITERS) == false) [[unlikely]]	// 1: desired value: locked, no other waiting
		{               // == false : is already locked, c contains found value
			result = wait_timeout(expected_found, timeout_relative);    // wait for futex, value is now 1 (LOCKED_NOWAITERS) or 2 (LOCKED_WAITERS)
		}
		return result;
	}
	...
	{								// value before = 2 (LOCKED_WAITERS) or 1 (LOCKED_NOWAITERS)
		...
		if(value.fetch_sub<AtomicMemoryOrder::RELEASE>(1) == State::LOCKED_WAITERS) [[unlikely]]		// is someone waiting?
